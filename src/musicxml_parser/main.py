...
class MusicXMLDocument(object):
    ...
    def _parse(self):
        ...
        if xml_part_list is not None:
            for element in xml_part_list:
                if element.tag == "score-part":
                    ...
                    element_text = ET.tostring(element, encoding="utf-8").decode(
                        "utf-8"
                    )
                    ...
                    if any(
                        ...
                        for keyword in constants.DRUM_PART_KEYWORDS
                    ):
                        # continue
                        is_drum_part = True
        ...
        for score_part_index, child in enumerate(self._score.findall("part")):
            part = Part(
                child, self._score_parts, self._state, expandRepeats=self.expandRepeats
            )
    ...
    def _recalculate_time_position(self):
        ...
        for i in range(len(tempos)):
            ...
            if i + 1 < len(tempos):
                new_time_position += (
                    (tempos[i + 1].xml_position - tempos[i].xml_position)
                    / tempos[i].qpm
                    * 60
                    / tempos[i].state.divisions
                )
            ...
            for measure in part.measures:
                for note in measure.notes:
                    for i in range(len(tempos)):
                        if i + 1 == len(tempos):
                            current_tempo = (
                                tempos[i].qpm / 60 * tempos[i].state.divisions
                            )
                            break
                        else:
                            if (
                                tempos[i].xml_position
                                <= note.note_duration.xml_position
                                and tempos[i + 1].xml_position
                                > note.note_duration.xml_position
                            ):
                                current_tempo = (
                                    tempos[i].qpm / 60 * tempos[i].state.divisions
                    ...
                    note.note_duration.time_position = (
                        tempos[i].time_position
                        + (note.note_duration.xml_position - tempos[i].xml_position)
                        / current_tempo
                    )
    ...
    def get_notes(self, melody_only=False, grace_note=True):
        ...
        for instrument_index in range(num_parts):
            ...
            notes_part, rests_part = get_playable_notes(
                part,
        ...
        notes.sort(
            key=lambda x: (
                x.note_duration.xml_position,
                x.note_duration.grace_order,
                -x.pitch[1],
            )
        )
        return notes, rests
    ...
    def get_part_notes(
        self, part, melody_only=False, grace_note=True, ignore_slurs=False
    ):
    ...
    def get_beat_positions(self, in_measure_level=False, part_index=0):
        ...
        for i in range(num_measure):
            ...
            full_measure_length = (
                corresp_time_sig.state.divisions
                * corresp_time_sig.numerator
                / corresp_time_sig.denominator
                * 4
            )
            if i < num_measure - 1:
                actual_measure_length = (
                    piano.measures[i + 1].start_xml_position - measure_start
                )
            else:
            ...
            if measure.implicit:
                ...
                if i == 0:
                    for j in range(-num_beat_in_measure, 0):
                        beat = (
                            piano.measures[i + 1].start_xml_position
    ...
    def get_interval_positions(self, interval_in_16th):
        ...
        for i in range(num_measure):
            ...
            num_beat_in_measure = int(
                corresp_time_sig.numerator
                * 16
                / (corresp_time_sig.denominator * interval_in_16th)
            )
            if num_beat_in_measure == 0:
                raise Exception(
                    f"measure is longer than a interval: {corresp_time_sig.numerator} / {corresp_time_sig.denominator}"
                )
            inter_beat_interval = (
                corresp_time_sig.state.divisions * interval_in_16th / 4
            )
    ...
    def get_accidentals(self):
        ...
        for part in self.parts:
            for measure in part.measures:
                for direction in measure.directions:
                    if (
                        direction.type["type"] == "words"
                        and direction.type["content"] in accs
                    ):
                        directions.append(direction)
        return directions
...
class ReductedMusicXMLDocument(MusicXMLDocument):
    ...
    def _parse(self):
        ...
        guitarPartsET = []
        for i, part in enumerate(self.predictions["parts"]):
            guitarPartET = ET.fromstring(
                f"""<part id="T{i+1}">
                </part>
                """
            )
            guitarPartsET.append(guitarPartET)

            guitarScorePart = ET.fromstring(
                f"""<score-part id="T{i+1}">
                <part-name print-object="yes">{part["name"]}</part-name>
                <score-instrument id="T{i+1}-I2">
                    <instrument-name>Guitar</instrument-name>
                    </score-instrument>
                <midi-device id="T{i+1}-I2" port="1"></midi-device>
                <midi-instrument id="T{i+1}-I2">
        ...
        [self._score.append(guitarPart) for guitarPart in guitarPartsET]
